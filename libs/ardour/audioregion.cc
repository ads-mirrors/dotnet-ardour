                      ...
                      uint32_t    chan_n) const
{
	...
	*/
	...
	if (n_chn > 1 && _cache_start < _cache_end && internal_offset >= _cache_start && internal_offset + to_read + _cache_tail <= _cache_end) {
		...
		copy_vector (mixdown_buffer, _readcache.get_audio (chan_n).data (internal_offset - _cache_start), to_read + _cache_tail);
		cl.release ();
	} else {
		...
		_cache_end   = internal_offset + to_read + n_tail;
		_cache_tail  = n_tail;
		cl.release ();
	}
	...
	if (_cache_tail > 0) {
		...
		mix_buffers_no_gain (buf + to_read, mixdown_buffer + to_read, _cache_tail);
	}

	return to_read + _cache_tail;
}
