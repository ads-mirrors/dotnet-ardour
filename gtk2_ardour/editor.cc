...
#warning Fix Properties Sidebar Layout to fit < 720px height
	...
	content_list_pane.set_drag_cursor (*_cursors->expand_left_right);
	...
	content_list_ebox.add (_editor_list_vbox);
	content_toolbar_ebox.add (global_vpacker);
	content_innermost_ebox.add (editor_summary_pane);
	...
	if (ev->type == GDK_2BUTTON_PRESS) {
		...
		if (_notebook_shrunk) {
			if (pre_notebook_shrink_pane_width) {
				content_list_pane.set_divider (0, *pre_notebook_shrink_pane_width);
			}
			_notebook_shrunk = false;
		} else {
			pre_notebook_shrink_pane_width = content_list_pane.get_divider();

			/* this expands the LHS of the edit pane to cover the notebook
			   PAGE but leaves the tabs visible.
			 */
			content_list_pane.set_divider (0, content_list_pane.get_divider() + page->get_width());
...
Editor::ui_parameter_changed (string parameter)
{
	if (parameter == "icon-set") {
		...
		content_list_pane.set_drag_cursor (*PublicEditor::instance().cursors()->expand_left_right);
		editor_summary_pane.set_drag_cursor (*_cursors->expand_up_down);
